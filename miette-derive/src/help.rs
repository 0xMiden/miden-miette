            ...
            |ident, fields, DiagnosticConcreteArgs { help, .. }| {
                ...
                match &help.as_ref()? {
                    Help::Display(display) => {
                        ...
                        Some(quote! {
                            Self::#ident #display_pat => core::option::Option::Some(Box::new(format!(#fmt #args))),
                        })
                    }
                    Help::Field(member, ty) => {
                        ...
                        Some(quote! {
                            Self::#ident #display_pat => {
                                ...
                                miette::macro_helpers::OptionalWrapper::<#ty>::new().to_option(&#help).as_ref().map(|#var| -> Box<dyn core::fmt::Display + '_> { Box::new(format!("{}", #var)) })
                            },
                        })
                    }
                }
            ...
            Help::Display(display) => {
                ...
                Some(quote! {
                    fn help(&self) -> core::option::Option<Box<dyn core::fmt::Display + '_>> {
                        ...
                        core::option::Option::Some(Box::new(format!(#fmt #args)))
                    }
                })
            }
            Help::Field(member, ty) => {
                ...
                Some(quote! {
                    fn help(&self) -> core::option::Option<Box<dyn core::fmt::Display + '_>> {
                        ...
                        let Self #display_pat = self;
                        use miette::macro_helpers::ToOption;
                        miette::macro_helpers::OptionalWrapper::<#ty>::new().to_option(&self.#member).as_ref().map(|#var| -> Box<dyn core::fmt::Display + '_> { Box::new(format!("{}", #var)) })
                    }
                })
            }
        }
